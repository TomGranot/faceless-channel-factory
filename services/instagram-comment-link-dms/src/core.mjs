...
export async function runCycle({posts, channels, listComments, sendPrivateReply, state, saveState, writesEnabled, maxWrites}) {
  const campaigns = publishedCampaigns(posts, channels);
  const summary = {campaigns: campaigns.length, commentsRead: 0, matched: 0, sent: 0, uncertain: 0, rejected: 0, deduplicated: 0, writesDisabled: 0};
  let writes = 0;
  for (const campaign of campaigns) {
    ...
    for (const comment of comments) {
      ...
      try {
        ...
        state.comments[key] = {...state.comments[key], status: "sent", providerMessageId: result.id, completedAt: new Date().toISOString()};
        summary.sent += 1;
      } catch (error) {
        const status = error?.ambiguous ? "uncertain" : "rejected";
        state.comments[key] = {...state.comments[key], status, errorCode: String(error?.code || "provider_error"), completedAt: new Date().toISOString()};
        summary[status] += 1;
      }
      await saveState(state);
    }
