...
When a macOS LaunchAgent runs the dispatcher, pass the user session's `SSH_AUTH_SOCK` through the same operational environment allowlist used for the repair child. Do not hardcode the per-login socket path or restore unrelated inherited credentials. Test one non-interactive SSH read from the installed LaunchAgent before claiming automated recovery works.
